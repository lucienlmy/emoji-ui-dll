...
namespace EmojiWindowDemo
{
    internal static class EditBoxDemoPage
    {
        public static void Build(AllDemoShell shell, IntPtr page)
        {
            ...
            app.GroupBox(16, 16, 980, 248, "⌨️ EditBox 实时属性读取", DemoTheme.Border, DemoTheme.Background, page);
            app.GroupBox(16, 286, 980, 254, "🪄 文本 / 颜色 / 尺寸 / 状态", DemoTheme.Border, DemoTheme.Background, page);
            app.GroupBox(1020, 16, 444, 524, "📄 多行 EditBox 演示", DemoTheme.Border, DemoTheme.Background, page);
            ...
            IntPtr memo = EmojiWindowNative.CreateEditBox(page, 1044, 72, 396, 244, memoText, memoText.Length, DemoTheme.Text, DemoTheme.Background, font, font.Length, 13, 0, 0, 0, PageCommon.AlignLeft, 1, 0, 0, 1, 0);
            IntPtr readout = app.Label(40, 184, 920, 88, "等待读取编辑框属性。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, true, page);
            IntPtr stateLabel = app.Label(40, 760, 920, 22, "编辑框页状态将在这里更新。", DemoTheme.Primary, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
            app.Label(40, 56, 900, 24, "这一页直接读取文本、颜色、位置、字体、启用态和可见态，不再只是放一个输入框。", DemoTheme.Muted, DemoTheme.Background, 12, PageCommon.AlignLeft, true, page);
            app.Label(1044, 336, 380, 52, "右侧只保留多行编辑框和布局/状态控制，不再混入无关标签演示，让层次更接近 Python v2。", DemoTheme.Muted, DemoTheme.Background, 12, PageCommon.AlignLeft, true, page);
            ...
            app.Label(40, 598, 900, 24, "1. GetEditBoxText / SetEditBoxText：读取和修改输入文本。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
            app.Label(40, 632, 900, 24, "2. GetEditBoxColor / SetEditBoxColor：读取和切换前景色 / 背景色。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
            app.Label(40, 666, 900, 24, "3. GetEditBoxBounds / SetEditBoxBounds：直接修改编辑框位置与宽度。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
            app.Label(40, 700, 900, 24, "4. GetEditBoxFont / SetEditBoxFont：读取和切换字体名、字号和粗体。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
            app.Label(40, 734, 900, 24, "5. EnableEditBox / ShowEditBox：演示启用态和可见态切换。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
