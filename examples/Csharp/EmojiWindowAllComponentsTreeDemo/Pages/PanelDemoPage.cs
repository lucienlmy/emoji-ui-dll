    ...
    internal static class PanelDemoPage
    {
        public static void Build(AllDemoShell shell, IntPtr page)
        {
            ...
            IntPtr detailLabel = app.Label(
                ...
                1368,
                50,
                "Panel 适合把同一业务域的控件组织成一个视觉整体，既能当背景容器，也能承担布局分区。",
                DemoTheme.Text,
                DemoTheme.Background,
                12,
                PageCommon.AlignLeft,
            ...
            app.Label(40, 718, 760, 24, "2. Panel 适合承载导航、摘要卡、工具区、活动流这类强语义区域。", DemoTheme.Text, DemoTheme.Background, 12, PageCommon.AlignLeft, false, page);
            ...
            app.Label(18, 148, 170, 22, "⚙️ 设置中心", DemoTheme.Muted, DemoTheme.Background, 12, PageCommon.AlignLeft, false, sidebar);
            ...
            app.Label(18, 134, 314, 40, "容器内的数字卡片、摘要信息、统计指标很适合放进独立 Panel。", DemoTheme.Muted, DemoTheme.Surface, 12, PageCommon.AlignLeft, true, stats);
            ...
            app.Label(18, 142, 662, 28, "当前建议：把操作按钮和说明文字放到同一块 Panel 里，用户更容易理解这是一个功能区。", DemoTheme.Muted, DemoTheme.SurfacePrimary, 12, PageCommon.AlignLeft, true, actions);

            app.Label(18, 16, 180, 24, "📰 活动流 Panel", DemoTheme.Text, DemoTheme.Background, 14, PageCommon.AlignLeft, false, feed);
            app.Label(18, 56, 440, 20, "🕘 10:20  已完成主题切换兼容修复", DemoTheme.Muted, DemoTheme.Background, 12, PageCommon.AlignLeft, false, feed);
            app.Label(18, 86, 440, 20, "🕘 10:32  已补全 Tooltip 四方向与主题展示", DemoTheme.Muted, DemoTheme.Background, 12, PageCommon.AlignLeft, false, feed);
            ...
            IntPtr feedTip = app.Label(
                ...
                548,
                74,
                "这一块模拟的是消息流、日志流或最近活动区域。Panel 在这里最重要的作用，是把连续文本和操作上下文打包成一个稳定区块。",
                DemoTheme.Muted,
                DemoTheme.Background,
                12,
                PageCommon.AlignLeft,
            ...
            void ApplyTheme()
            {
                ...
                EmojiWindowNative.SetPanelBackgroundColor(sidebar, palette.PageBackground);
                EmojiWindowNative.SetPanelBackgroundColor(stats, palette.CardBackground);
                EmojiWindowNative.SetPanelBackgroundColor(actions, palette.Dark ? EmojiWindowNative.ARGB(255, 39, 56, 88) : DemoColors.LightBlue);
                EmojiWindowNative.SetPanelBackgroundColor(feed, palette.PageBackground);
                shell.SetLabelText(intro, palette.Dark
                    ? "深色主题下，Panel 仍保持侧栏、摘要、快捷操作区和活动流四段式结构。"
                    : "浅色主题下，Panel 页保持侧栏、摘要、快捷操作区和活动流四段式结构，更接近 Python 版。");
                shell.SetLabelText(feedTip, palette.Dark
                    ? "深色主题里活动流这块负责承接连续文本和上下文，避免主舞台只剩下几块色板。"
                    : "浅色主题里活动流这块负责承接连续文本和上下文，页面层次会更完整。");
            }
